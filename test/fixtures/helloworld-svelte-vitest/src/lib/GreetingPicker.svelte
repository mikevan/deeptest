<script module lang="ts">
  /**
   * The villain, inside a Svelte component's module script.
   *
   ...
   * It lives in a .svelte file on purpose: the toolkit has to read the script
   * blocks of a single-file component to find it.
   */
...
<script lang="ts">
  let { hour, name, lang = 'en', formal = false, mood = 'ok' }: { hour: number; name: string; lang?: string; formal?: boolean; mood?: string } = $props();
  const text = $derived(pickGreeting(hour, name, lang, formal, mood));
</script>
