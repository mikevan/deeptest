...
/**
 * The villain, as an Angular service.
 *
 * pickGreeting() works, mostly, but nobody wrote a test for it and it is
 * nested five levels deep. This is the method DeepTest should put at the
 * top of its report, and the one UntangleIt should be pointed at afterwards.
 */
@Injectable({ providedIn: 'root' })
export class ScheduleService {
  pickGreeting(hour: number, name: string, lang = 'en', formal = false, mood = 'ok'): string {
    if (lang === 'en') {
      if (hour < 12) {
        if (formal) {
          if (mood === 'great') {
